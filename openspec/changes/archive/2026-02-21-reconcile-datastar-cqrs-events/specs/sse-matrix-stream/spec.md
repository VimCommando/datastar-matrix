...
#### Scenario: Command acknowledgement and stream state are decoupled
- **WHEN** a command is accepted through `/cmd/*` and the server returns `204 No Content`
- **THEN** the next observable UI state change is delivered through `/events` stream updates rather than the command response
